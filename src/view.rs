...
use crate::settings::*;
use sorts::SortingStateEnum;

fn draw_hood(draw: &Draw, win: Rect<f32>, model: &Model) {
    let hood = Rect::from_w_h(win.w(), win.h() * 0.18).top_left_of(Rect::from(win));
    draw.rect().xy(hood.xy()).wh(hood.wh()).color(named::PURPLE);
    draw.text("Press 'H' for help")
        .xy(hood.top_left() + vec2(70.0, -10.0))
        .font_size(16)
        .color(named::WHITE);
...
fn draw_sorting_states(draw: &Draw, win: Rect<f32>, model: &Model) {
    ...
    for (i, val) in model.sorter.get_states()[model.index].iter().enumerate() {
        let r = Rect::from_w_h(width_of_rect, height_of_rect * (val.value as f32))
            ...
            .pad(1.0);
        draw.rect().xy(r.xy()).wh(r.wh()).color(match val.state {
